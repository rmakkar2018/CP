...
void func(){
	...
	// for(auto it=count_nums.begin();it!=count_nums.end();++it){
	// 	cout<<it->first<<" "<<it->second<<endl;
	// }

	bool ans=true;

	vector<int> start_cols(n);
	start_cols[0]=minn;
	for(int i=1;i<n;i++){
		if(count_nums.find(start_cols[i-1]+d) !=count_nums.end()&& count_nums[start_cols[i-1]+d]>0){
			count_nums[start_cols[i-1]+d]--;
			start_cols[i]=start_cols[i-1]+d;
		}else{
			ans=false;
			break;
		}
	...
	if(ans){
		for(int i=0;i<n;i++){
			...
			for(int j=1;j<n;j++){
				int val=start+j*c;
				if(count_nums.find(val)!=count_nums.end() && count_nums[val]>0){
					count_nums[val]--;
				}else{
					ans=false;
					break;
